...
def plot_vectorized_optimized(WIDTH=1500, HEIGHT=1500, MAX_ITER=80):
    start = timer()
    n = mandelbrot_vectorized(WIDTH=WIDTH, HEIGHT=HEIGHT, MAX_ITER=MAX_ITER)
    end = timer()
...
def plot_looped_optimized(WIDTH=1500, HEIGHT=1500, MAX_ITER=80):
    start = timer()
    n = mandelbrot_looped(WIDTH=WIDTH, HEIGHT=HEIGHT, MAX_ITER=MAX_ITER)
    end = timer()
    plt.figtext(0.5, 0.01, "Time to run: {:.2f} seconds".format(end-start), ha="center",
                fontsize=14, bbox={"facecolor": "white", "alpha": 0.5, "pad": 4})
    plt.imshow(n, cmap='magma')
